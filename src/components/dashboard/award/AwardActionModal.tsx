...
import { AwardAction, AwardRequest } from "@/types/award.types"

interface AwardActionModalProps {
  isOpen: boolean
...
const ACTION_LABELS: Record<AwardAction, string> = {
  submit: "Submit",
  approve: "Approve",
  reject: "Reject",
  return: "Return",
...
export function AwardActionModal({ isOpen, actionType, request, onClose, onConfirm }: AwardActionModalProps) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg border border-slate-200 shadow-xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-slate-600 leading-6">
            You are about to <span className="font-semibold text-slate-900">{ACTION_LABELS[actionType].toLowerCase()}</span> award request
            <span className="font-semibold text-slate-900"> {request.id}</span> for
            <span className="font-semibold text-slate-900"> {request.employee.employeeName}</span>.
          </p>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Add comments for the audit trail or decision summary..."
