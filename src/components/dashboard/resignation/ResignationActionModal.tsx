...
import { ResignationRequest, WorkflowAction } from "@/types/resignation.types"

interface ResignationActionModalProps {
  isOpen: boolean
...
export function ResignationActionModal({
  isOpen,
  actionType,
  request,
  onClose,
  onConfirm,
}: ResignationActionModalProps) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg border border-slate-200 shadow-xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-slate-600 leading-6">
            You are about to <span className="font-semibold text-slate-900">{ACTION_LABELS[actionType].toLowerCase()}</span> resignation request
