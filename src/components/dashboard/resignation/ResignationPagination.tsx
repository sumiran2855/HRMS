...
import { RESIGNATION_DATA, RESIGNATION_PAGE_SIZE_OPTIONS } from "@/constants/resignation"

interface ResignationPaginationProps {
  totalPages: number
  currentPage: number
  rowsPerPage: number
...
export function ResignationPagination({
  totalPages,
  currentPage,
  rowsPerPage,
  setRowsPerPage,
  goToPage,
  filteredCount,
}: ResignationPaginationProps) {
  if (totalPages <= 1 && filteredCount <= RESIGNATION_PAGE_SIZE_OPTIONS[0]) return null
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-4">
            <p className="text-xs text-slate-500">
              ...
              {filteredCount !== RESIGNATION_DATA.length && (
                <span className="text-slate-400"> (filtered from {RESIGNATION_DATA.length} total)</span>
              )}
            </p>
            <div className="flex items-center gap-1.5">
              ...
              >
                {RESIGNATION_PAGE_SIZE_OPTIONS.map((size) => (
