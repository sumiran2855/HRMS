      ...
      <div className="flex h-21 items-center px-6 lg:px-8 gap-6">
        ...
        <div className="hidden sm:block">
          <h2 className="text-base font-semibold text-foreground tracking-tight">
            Hello, <span className="text-primary font-bold">{user?.fullName?.split(' ')[0] || 'User'}</span>
            ...
            <DropdownMenuContent className="w-54 mt-4 rounded-xl border-border/50 shadow-lg py-2 px-3 ml-10" align="end">
              <DropdownMenuLabel className="font-normal py-3">
                <div className="flex flex-col space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground font-medium">{user?.email || 'user@company.com'}</p>
                </div>
              </DropdownMenuLabel>
              ...
              <DropdownMenuSeparator className="bg-border/50" />
              <DropdownMenuItem
                className="cursor-pointer py-2.5 rounded-lg mx-1 text-destructive focus:text-destructive focus:bg-destructive/10 transition-colors"
